...
void *handle_client(void *arg){
    ...
    send_message_self("Connected", cli->sockfd);
                ...
                if (param) {
                    ...
                    if (param) {
                        ...
                        if(i == 1){
                             ...
                             int s_time = mytime->tm_hour*3600 + mytime->tm_hour*60 + mytime->tm_sec;
                             printf("%d\n",s_time );
                             send_time = s_time;
			                 lansw = strToInt(s);
                             printf("%d\n",lansw);
                        ...
                        if(i == 2){
                            ...
                            time(&currenttime);
                            struct tm * mytime = localtime(&currenttime);
                            int r_time = mytime->tm_hour*3600 + mytime->tm_hour*60 + mytime->tm_sec;
                            recv_time = r_time;
                            printf("%d\n",recv_time );
                            printf("%d\n",send_time );
                            int answer = strToInt(s);
                            if((recv_time - send_time) > 15){
                                printf("Time is over\n");
                                perror;
                            }
                            else if((recv_time - send_time) <= 15){
                                if(answer == lansw){
                                    send_message_client(buff_out, uid);
